...
import com.guggiemedia.fibermetric.ui.component.animation.DefaultAnimationHandler;
import com.guggiemedia.fibermetric.ui.component.animation.MenuAnimationHandler;
...
public class FloatingActionMenu {
    ...
    private List<View> subActionLabels;
    ...
    public FloatingActionMenu(final View mainActionView,
        ...
        this.subActionLabels = subActionLabels;
    ...
    private void addSubButtonLabel(int i) {
        View labelView = subActionLabels.get(i);

        final FrameLayout.LayoutParams params = (FrameLayout.LayoutParams) labelView.getLayoutParams();

        int width = subActionItems.get(i).width * 3;

        params.setMargins(subActionItems.get(i).x - width, subActionItems.get(i).y + 5, 0, 0);

        labelView.setLayoutParams(params);

        ((ViewGroup) getActivityContentView()).addView(labelView);
    }
...
//            detachOverlayContainer();
        ...
        for (int i = 0; i < subActionItems.size(); i++) {
            ...
            ((ViewGroup) getActivityContentView()).removeView(labelView);
        }
    ...
    private Point calculateItemPositions() {
        ...
        for (int i = 0; i < subActionItems.size(); i++) {
            ...
            subActionItems.get(i).y = center.y - radius - (i * (subActionItems.get(i).height));
        }
    ...
    public List<View> getSubActionLabels() {
        return subActionLabels;
    }
...
//            parent.removeView(view);
    ...
    public static class Builder {
        ...
        public Builder addSubActionLabel(int labelId, Context context) {
            TextView textView = new TextView(context);

            textView.setText(context.getResources().getString(labelId));

            textView.setTextColor(context.getResources().getColor(R.color.warm_grey_dark));

            Drawable background = context.getResources().getDrawable(R.drawable.rounded_corner);

            textView.setBackground(background);

            int height = context.getResources().getDimensionPixelSize(R.dimen.sub_action_label_height);
            int width = context.getResources().getDimensionPixelSize(R.dimen.sub_action_label_width);
        ...
        public Builder setParentViewGroup(ViewGroup parent) {
            this.parent = parent;
